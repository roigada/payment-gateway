...
ALTER TABLE payments
    ...
    ADD CONSTRAINT payments_status_private_fields_check CHECK (
        ...
        OR
        (status = 'authorized' AND bank_authorization_id IS NOT NULL AND bank_capture_id IS NULL AND capture_bank_operation_key IS NULL AND bank_refund_id IS NULL AND refund_bank_operation_key IS NULL AND decline_reason IS NULL AND bank_void_id IS NULL)
        OR
        ...
        OR
        (status = 'captured' AND bank_authorization_id IS NOT NULL AND bank_capture_id IS NOT NULL AND capture_bank_operation_key IS NOT NULL AND bank_refund_id IS NULL AND refund_bank_operation_key IS NULL AND decline_reason IS NULL AND bank_void_id IS NULL AND void_bank_operation_key IS NULL)
        OR
