...
func NewAuthorizationRetryClaim(key string, requestFingerprint string, paymentID domain.PaymentID, authorizationCardFingerprint string, now time.Time, claimStuckAfter time.Duration) PaymentCommandClaimRequest {
	return PaymentCommandClaimRequest{
...
func NewCaptureClaim(key string, requestFingerprint string, paymentID domain.PaymentID, bankOperationKey string, now time.Time, claimStuckAfter time.Duration) PaymentCommandClaimRequest {
	return PaymentCommandClaimRequest{
...
func NewVoidClaim(key string, requestFingerprint string, paymentID domain.PaymentID, bankOperationKey string, now time.Time, claimStuckAfter time.Duration) PaymentCommandClaimRequest {
	return PaymentCommandClaimRequest{
...
func NewRefundClaim(key string, requestFingerprint string, paymentID domain.PaymentID, bankOperationKey string, now time.Time, claimStuckAfter time.Duration) PaymentCommandClaimRequest {
	return PaymentCommandClaimRequest{
...
func (s *PaymentService) AuthorizePayment(ctx context.Context, command AuthorizePaymentCommand) (result PaymentCommandResult, err error) {
	...
	execution, err := s.commandExecutor().execute(
		ctx,
		NewAuthorizationStartClaim(command.idempotencyKey, fingerprint, payment, now, s.claimStuckAfter),
		func(ctx context.Context, payment *domain.Payment) (PaymentCommandResult, error) {
			return s.authorizeClaimedPayment(ctx, payment, command.card, 201)
		},
	)
...
func (s *PaymentService) RetryAuthorization(ctx context.Context, command RetryAuthorizationCommand) (result PaymentCommandResult, err error) {
	...
	execution, err := s.commandExecutor().execute(
		...
		func(ctx context.Context, payment *domain.Payment) (PaymentCommandResult, error) {
			return s.authorizeClaimedPayment(ctx, payment, command.card, 200)
		},
	)
	if execution.replayed {
		outcomeOverride = paymentOperationOutcomeReplayed
...
func (s *PaymentService) authorizeClaimedPayment(ctx context.Context, payment *domain.Payment, card cardDetails, httpStatus int) (PaymentCommandResult, error) {
...
func (s *PaymentService) commandExecutor() paymentCommandExecutor {
	return paymentCommandExecutor{store: s.store, metrics: s.operationMetrics, clock: s.clock}
}
...
func (s *PaymentService) captureClaimedPayment(ctx context.Context, payment *domain.Payment) (PaymentCommandResult, error) {
	...
	if err != nil {
		if HasPaymentErrorKind(err, PaymentErrorAuthorizationExpired) {
			return authorizationExpiredCompletion(payment, s.clock.Now())
		}
		return PaymentCommandResult{}, ensurePaymentError(err)
	}
...
func (s *PaymentService) voidClaimedPayment(ctx context.Context, payment *domain.Payment) (PaymentCommandResult, error) {
...
func (s *PaymentService) refundClaimedPayment(ctx context.Context, payment *domain.Payment) (PaymentCommandResult, error) {
...
func authorizationExpiredCompletion(payment *domain.Payment, now time.Time) (PaymentCommandResult, error) {
	...
	return PaymentCommandResult{}, newCompletedPaymentCommandError(result, NewPaymentAuthorizationExpiredError(nil))
}
