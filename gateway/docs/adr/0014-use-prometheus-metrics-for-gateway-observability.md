...
Prometheus and Grafana are external observability infrastructure, not gateway application dependencies.

Prometheus intentionally scrapes only the payment gateway `/metrics` endpoint. It must not scrape the Mock Bank directly. Operationally, the gateway treats the Mock Bank as an External Bank Dependency; the gateway can observe dependency request rate, result, and latency through outbound `mock_bank_*` metrics, but it does not inspect the bank's internals.
...
Gateway-owned custom metrics use the `payment_gateway_` prefix so their service ownership is clear. Standard Go runtime and process collectors keep their conventional `go_*` and `process_*` names.
...
Current Pending Payment visibility is collected from the gateway's Payment store at scrape time as aggregate gauges: `payment_gateway_pending_payments_total` and `payment_gateway_oldest_pending_payment_age_seconds`. These metrics have no labels, and the oldest age is zero when no Pending Payments exist. The collector is read-only: it does not change Payment Status and does not call the Mock Bank. Resolution remains client-driven through Authorization Retry; the observability path neither stores raw card details or CVV nor initiates bank calls.
